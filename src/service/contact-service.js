...
const remove = async (user, contactId) => {
    ...
    if (totalInDatabase !== 1) throw new ResponseError(404, 'Contact is not found')
...
const search = async (user, request) => {
    ...
    if (request.phone) filters.push({
        phone: {
            contains: request.phone
        }
    })

    if (request.email) filters.push({
        email: {
